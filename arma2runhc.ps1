...
Set-Location "$PSScriptRoot\arma2\33905"
for ($i = 1; $i -le [int]$args[0]; $i++) {
  ...
  $hcProcess = Start-Process -FilePath "ArmA2Server.exe" -ArgumentList "-client", "-nosound", "-profiles=A2Master", "-connect=${connect}:$($args[2])", "-port=$basePort", "-password=`"$($args[3])`"", "`"-mod=$($args[4])`"", "`"-par=$parfile`"" -WindowStyle Hidden -PassThru
